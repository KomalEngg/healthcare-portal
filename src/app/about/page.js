    ...
    <div className="w-full bg-white font-sans text-gray-800">
      ...
      <section className="w-full bg-[#1a1a1a] py-24 px-6 md:px-16 text-center text-white relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto relative z-10 space-y-5">
          <p className="text-sm tracking-[0.3em] uppercase font-bold text-[#e11d48]">
            Your Trusted Healthcare Partner
          </p>
          <h1 className="text-[44px] md:text-[60px] font-serif leading-tight uppercase">
            About Alhawat <span className="text-[#e11d48]">Medical</span>
          </h1>
          <div className="pt-4 flex items-center justify-center gap-3 text-sm font-medium">
            <Link href="/" className="text-gray-400 hover:text-[#e11d48] transition">Home</Link>
        ...
        <div className="max-w-7xl mx-auto space-y-16">
          <div className="flex flex-col md:flex-row items-center gap-12 lg:gap-20">
            <div className="flex-1 space-y-6">
              <p className="text-xs tracking-widest uppercase font-bold text-[#e11d48]">About Us</p>
            ...
            <div className="w-full md:w-[45%] aspect-video bg-white rounded-[40px] shadow-2xl flex items-center justify-center border-[10px] border-white group cursor-pointer relative overflow-hidden"
              ...
              <div className="relative z-10 w-20 h-20 bg-black/60 rounded-full flex items-center justify-center border-2 border-white text-white text-2xl group-hover:bg-[#e11d48] group-hover:border-[#e11d48] transition-all duration-500 shadow-xl">▶</div>
            </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 text-center pt-10">
            {aboutData.stats.map((item, idx) => (
              <div key={idx} className="bg-white p-10 rounded-[30px] shadow-sm border border-gray-100 group hover:border-[#e11d48]/30 transition-all duration-500">
                <h3 className="text-[50px] md:text-[64px] font-serif text-[#e11d48] leading-none group-hover:scale-110 transition-transform">{item.stat}</h3>
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-16">
          ...
          <div className="w-full lg:w-1/2 space-y-12 relative z-10">
            <div className="space-y-4 text-center lg:text-left">
              <p className="text-xs tracking-[0.4em] uppercase font-bold text-[#e11d48]">Our Process</p>
              <h2 className="text-[36px] md:text-[54px] font-serif leading-tight uppercase">How It <span className="text-[#e11d48]">Works</span></h2>
            </div>
            
            <div className="space-y-10">
              {aboutData.process.map((step, idx) => (
                <div key={idx} className="flex gap-8 items-start group">
                  <div className="text-[40px] font-serif text-[#e11d48] opacity-20 group-hover:opacity-100 transition-opacity leading-none pt-1">
                    {step.n}
                  </div>
                  <div className="flex-1 space-y-2 border-b border-white/10 pb-6">
                    <h4 className="text-2xl font-serif text-white group-hover:text-[#e11d48] transition-colors uppercase tracking-tight">{step.t}</h4>
      ...
      <section className="w-full bg-white py-24 px-6 md:px-16 lg:px-24 text-center">
        <div className="max-w-7xl mx-auto space-y-16">
          <div className="space-y-4">
             <span className="inline-block bg-[#1a1a1a] text-[#e11d48] text-[10px] font-bold px-6 py-2 rounded-full uppercase tracking-[0.3em]">Our Professionals</span>
             <h2 className="text-[36px] md:text-[50px] font-serif text-black leading-tight uppercase">Experts Behind The <span className="text-[#e11d48]">Care</span></h2>
          </div>
            ...
            {aboutData.team.map((member, idx) => (
              <div key={idx} className="bg-[#fdf2e9] p-8 rounded-[40px] border border-gray-50 text-center group hover:shadow-2xl transition-all duration-500">
                <div className="relative w-32 h-32 mx-auto mb-6">
                    ...
                    <div className="absolute inset-0 rounded-full border-2 border-dashed border-[#e11d48] animate-spin-slow opacity-0 group-hover:opacity-40 transition-opacity"></div>
                </div>
                <h4 className="text-xl font-serif text-gray-900 mb-1">{member.name}</h4>
                <p className="text-[11px] text-[#e11d48] font-bold uppercase tracking-widest">{member.role}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="w-full bg-[#fdf2e9] py-24 px-6 md:px-16 lg:px-24">
        <div className="max-w-7xl mx-auto space-y-12">
          <div className="text-center space-y-4">
            <span className="inline-block bg-[#1a1a1a] text-[#e11d48] text-[10px] font-bold px-6 py-2 rounded-full uppercase tracking-[0.3em]">News & Articles</span>
            <h2 className="text-[36px] md:text-[50px] font-serif text-black leading-tight uppercase">Latest <span className="text-[#e11d48]">Blog Posts</span></h2>
          </div>
            ...
            {aboutData.blogs.map((post, idx) => (
              <div key={idx} className="bg-white rounded-[40px] overflow-hidden shadow-sm group border border-gray-100 flex flex-col hover:shadow-2xl transition duration-500">
                <div className="aspect-[4/3] overflow-hidden relative">
                  ...
                  <div className="absolute top-6 left-6 bg-[#1a1a1a] text-[#e11d48] text-[10px] font-bold px-4 py-2 rounded-full uppercase">Pharmacy</div>
                </div>
                <div className="p-10 space-y-5 flex-1 flex flex-col justify-between">
                  <h4 className="text-2xl font-serif text-gray-900 group-hover:text-[#e11d48] transition-colors leading-snug uppercase tracking-tight">{post.title}</h4>
                  <Link href={`/blog/${post.id}`} className="text-[11px] font-bold text-[#e11d48] uppercase tracking-[0.3em] flex items-center gap-2 group-hover:gap-4 transition-all">
